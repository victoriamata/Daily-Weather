...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    if (!cityName) {
      //in case city name wasn't entered: client error message
...
router.get("/history", async (_req: Request, res: Response) => {
  try {
    ...
    //get cities from historyService
    res.status(200).json(cities); // successful response
  } catch (error) {
...
router.delete("/history/:id", async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: "Error; unable to remove city.", error }); // server error response
  }
});
